...
class campaign_new_text_assets extends AWEBP_Page {

	function campaign_new_text_assets() {
	...
	function process(&$smarty) {
		...
		$smarty->assign("content_template", "campaign_new_text.htm");
		...
		campaign_save_markpos("text", $campaignid);
		...
		if (count($messagelist) > 1) {
			...
			if ($message["text"] == "")
				$message["text"] = message_html2text(message_select_row($messagelist[0]));
			...
			if ($campaign["basetemplateid"] > 0) {
				$message["text"] = (string)adesk_sql_select_one("SELECT content FROM #template WHERE id = '$campaign[basetemplateid]'");
			} elseif ($campaign["basemessageid"] > 0) {
				$message["text"] = (string)adesk_sql_select_one("SELECT text FROM #message WHERE id = '$campaign[basemessageid]'");
			} else {
			...
			*/
			...
			if ( !isset($message['format']) ) {
				$message['format'] = 1 /* check here if they selected html as well? */ ? 'mime' : 'text';
			}
		...
		if ($campaign["managetext"] == 0 && $campaign["type"] != "text") {
			# Just save the text form and move on.
			$up = array(
				"text" => $message["text"],
			);

			adesk_sql_update("#message", $up, "id = '$message[id]'");
			// clear all message sources for this campaign
			campaign_source_clear(null, $message['id'], null);
			adesk_http_redirect("desk.php?action=campaign_new_summary&id=$campaignid");
		}
	...
	function formProcess(&$smarty) {
		...
		if ($GLOBALS["campaign_save_id"] > 0)
			adesk_http_redirect("desk.php?action=campaign_new_text&id=$GLOBALS[campaign_save_id]");
	}
}
