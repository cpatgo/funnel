...
function campaign_validate(aftersave) {
	if ( aftersave == "next" ) {
		// Only need to check if one of these fields is hidden...
		if ($("campaign_fromname").type != "hidden" && ($("campaign_fromname").value == "" || $("campaign_fromemail").value == "")) {
			alert(campaign_message_str_noinfo);
			return false;
		}

		$("campaign_fromemail").value = adesk_str_trim($("campaign_fromemail").value);
		if ($("campaign_fromemail").type != "hidden" && !adesk_str_email($("campaign_fromemail").value)) {
			alert(campaign_message_str_nomail_from);
			$("campaign_fromemail").focus();
			return false;
		}
		$("campaign_reply2").value = adesk_str_trim($("campaign_reply2").value);
		if ($("campaign_reply2").value != '' && $("campaign_reply2").type != "hidden" && !adesk_str_email($("campaign_reply2").value)) {
			alert(campaign_message_str_nomail_reply2);
			$("campaign_reply2").focus();
			return false;
		}

		if ($("fetchwhen").value == "now" && $("campaign_subject").type != "hidden" && $("campaign_subject").value == "") {
