...
function report_list_subscription_select_row($id) {
	...
	return adesk_sql_select_row(report_list_subscription_select_query($so));
}
...
function report_list_subscription_select_array($so = null, $ids = null, $filter = 0) {
	...
	// fetch the rows
	//dbg(adesk_prefix_replace(report_list_subscription_select_query($so)));
	$rows = adesk_sql_select_array($q = report_list_subscription_select_query($so));
...
function report_list_subscription_select_array_paginator($id, $sort, $offset, $limit, $filter, $listid = 0) {
	...
	$so->count();
	$total = (int)adesk_sql_select_one(report_list_subscription_select_query($so, $listid));
