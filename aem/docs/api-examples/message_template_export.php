...
Description: Export message template as XML file (includes dependent files).
...
Parameters (* denotes requirement):
{
	...
	[*api_action] => message_template_export
	[*api_output] => xml, json, or serialize
	[ *ids[] ] => Array of ID(s) of each message template you want to export
	[*type] => Type of export: XML or HTML
}
...
Example response:
{
	...
	[result_message] => A custom message that appears explaining what happened. Example: Template(s) exported
...
$params = array(
	...
	'ids[1]'        => 1, // ID of the message template you wish to export
	//'ids[2]'      => 2, // another one?
  'type'         => 'xml', // use 'xml' or 'html' (XML includes dependent files/images)
);
