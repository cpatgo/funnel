<?php
/**
Title: View multiple (a list of) message templates.

Description: View many message templates with a single API call.
...
Parameters (* denotes requirement):
{
	...
	[*api_action] => message_template_list
	[*api_output] => xml, json, or serialize
	[*ids] => ID's (comma-separated) of each message template you want to view. Pass 'all' to return all templates.
}
...
Example response:
{
	[0] => Array
	  (
	    ...
	    [name] => My Template
	    [subject] => Subject
	    [content] => <html><head><title>Template added via API</title></head><body>This template was added via the API</body></html>
	    [format] => html
	    [categoryid] => 1
	    [preview_mime] =>
	    [preview_data] =>
...
$params = array(
	...
	// this is the action that fetches message template info based on the ID you provide
	'api_action'   => 'message_template_list',
	...
	// a comma-separated list of IDs of message templates you wish to fetch
	// pass 'all' to return all templates
	'ids'          => '1,2,3,4,5',
);
