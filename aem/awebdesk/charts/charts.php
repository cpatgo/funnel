...
function SendChartData($chart = array(), $additional = '') {
    require_once(dirname(dirname(__FILE__)) . '/functions/ajax.php');
	header("Pragma: public");
    adesk_ajax_print(ActualChartData($chart, $additional));
    //echo ActualChartData($chart, $additional);
}
function escapeURL($str) {
	require_once awebdesk_functions("str.php");
	return _charts_utf8( adesk_str_instr('&amp;', $str) ? $str : str_replace('&', '&amp;', $str) );
}

function _charts_utf8($text) {
	if (!function_exists("iconv"))
		return $text;
	return @iconv(_i18n("utf-8"), "UTF-8//IGNORE", $text);
}

//====================================
function ActualChartData( $chart=array(), $additional = '' ){
					...
					switch($Keys1[$i1]){
						...
						for($i3=0;$i3<count($Keys3);$i3++){
							switch(true){
								...
								default:
								// strip tags because HTML will throw off the graphs
								$xml.="\t\t\t<string>" . escapeURL($chart[$Keys1[$i1]][$Keys2[$i2]][$Keys3[$i3]]) . "</string>\r\n";
								break;
							}
						}
						$xml.="\t\t</row>\r\n";
						...
						for($i3=0;$i3<count($Keys3);$i3++){
							...
							else{$xml.="\t\t\t<string>" . escapeURL($chart[$Keys1[$i1]][$Keys2[$i2]][$Keys3[$i3]]) . "</string>\r\n";}
						}
						$xml.="\t\t</row>\r\n";
						break;
