...
class Swift
{
  ...
  function handshake(&$greeting)
  {
    ...
    if ($this->connection->getRequiresEHLO() || strpos($greeting->getString(), "ESMTP") || strpos($greeting->getString(), "extSMTP"))
  ...
  function send(&$message, $recipients, $from)
  {
    ...
    /* AwebDesk*/
    $stopCheck = 0;
    if ( isset($send_event->stopCheck) and (int)$send_event->stopCheck > 0 ) $stopCheck = (int)$send_event->stopCheck;
    if ( isset($GLOBALS['stopCheck'])  and (int)$GLOBALS['stopCheck']  > 0 ) $stopCheck = (int)$GLOBALS['stopCheck'];
    if ( $stopCheck > 0 ) {
      // should stop (1= this one, 2=all)
      if ( $stopCheck == 1 ) {
        // just return sent
        if ( function_exists('campaign_sender_log') ) campaign_sender_log("\n\n[+] SOFT STOP (SUBSCRIBER)\n");
        $this->reset();
        $this->notifyListeners($send_event, "SendListener");
        return 0;
      } else { // stopCheck=2
        // return sent and set a flag to stop the mailing completely
        if ( function_exists('campaign_sender_log') ) campaign_sender_log("\n\n[+] HARD STOP (MAILING)\n");
    ...
    /* AwebDesk*/
    if ( isset($this->getsource) and $this->getsource ) {
