    ...
    else
    {
      if (($this->name !== null))
      {
        if ( isset($GLOBALS['_hosted_account']) ) {
          if ( trim($this->name) )
          {
			  # PowerMTA actually adds quotes for us if there is a comma, so we 
			  # shouldn't add any ourselves.  (If we do, the email will go out 
			  # with two quotes in each position instead of just one, which is 
			  # even worse.)
