...
class Swift_Message_Encoder
{
  ...
  function &instance()
  {
    if (class_exists('SendingEngineEncoder')) return SendingEngineEncoder::instance();
   ...
   * @return string
   */
  function base64Encode($data, $chunk=76, $init_chunk=0, $headers=false, $le="\r\n")
  {
    $ret = "";
    $chunk -= 2;
    ...
    {
      $key = (string) $key;
      if (substr($key, 0, 1) == 'a') //This is an address
      {
   ...
   * @return string
   */
  function QPEncode($data, $chunk=72, $init_chunk=0, $headers=false, $le="\r\n")
  {
	  $chunk = 69;
    $ret = "";
    if ($headers) $data = $this->quoteChunk($data);
    ...
    {
      $key = (string) $key;
      if (substr($key, 0, 1) == 'a') //An address
      {
      ...
      else
      {
        ...
        while (preg_match($q = '/^.{1,'.($chunk-3).'}[^=]{2}(?!=[A-F0-9]{2})/s', $string, $matches)
          || preg_match($r = '/^.{1,'.($chunk-6).'}([^=]{0,3})?/s', $string, $matches)
          || (strlen($string) > 0 && $matches = array($string)))
        {
			# If the first character on the next line is a dot, then reduce
			# the length of the match by one and have that continue onto the
			# next line.  # So, instead of images.whatever\n.activehosted...,
			# it'd be # images.whateve\nr.activehosted...
			$first = substr($string, strlen($matches[0]), 1);
			if ($first == ".")
  ...
  function fixLE($data, $le)
  {
    ...
    if ($le != "\n") $data = str_replace("\n", $le, $data);
    //if ( isset($GLOBALS['_hosted_account']) ) return $data = preg_replace('/^\.$/', '..', $data);
