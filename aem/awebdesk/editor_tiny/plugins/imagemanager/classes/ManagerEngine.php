...
class Moxiecode_ManagerEngine {
	...
	function setConfig($config, $setup_values = true) {
		...
		if ($setup_values) {
			...
			if ($config["preview.urlprefix"]) {
				...
				if (isset($_SESSION["_hosted_account"])) {
					$config["preview.urlprefix"] = str_replace("{host}", $_SESSION[$_SESSION["_hosted_account"]]["account"], $config["preview.urlprefix"]);
				} else {
	...
	function getSiteRoot() {
		...
		if (isset($_SERVER["SCRIPT_NAME"]) && isset($_SERVER["SCRIPT_FILENAME"])) {
			$path = str_replace(strtolower($this->toUnixPath($_SERVER["SCRIPT_NAME"])), "", strtolower($this->toUnixPath($_SERVER["SCRIPT_FILENAME"])));
		...
		if (isset($_SERVER['DOCUMENT_ROOT']))
			...
			return $this->toUnixPath($_SERVER['DOCUMENT_ROOT']);

		// Check IIS' physical path
		if (isset($_SERVER['APPL_PHYSICAL_PATH']))
			return $this->toUnixPath(realpath($_SERVER['APPL_PHYSICAL_PATH']));
	...
	function convertPathToURI($abs_path, $root = false) {
		...
		if (isset($_SESSION["_hosted_account"])) {
			return str_replace("images/" . $_SESSION[$_SESSION["_hosted_account"]]["account"] . "/", "", $abs_path);
		}
