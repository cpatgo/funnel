	...
	elseif ( is_singular() ) {
		...
		if ( 'page' !== $post_type && 'post' !== $post_type ) {
			...
			if ( !empty( $post_type_object->has_archive ) && function_exists( 'get_post_type_archive_link' ) )
				...
				if($post_type == 'course' && $post_type_object->labels->name == 'Courses'):
					$trail[1] = '<a href="/library/" title="Library">Library</a>';
					$trail[2] = '<a href="/library/video-course-categories/" title="Video Course Categories">Video Course Categories</a>';

					$course_cat = array();
					$course_categories = get_the_terms($post_id, 'course_cat');
					foreach ($course_categories as $key => $value) {
						if(strpos(wp_get_referer(), $value->slug)) $course_cat = $value;
					}
					
					if(empty($course_cat) && !empty($course_categories)) $course_cat = $course_categories[0];

					if(!empty($course_cat)):
						$trail[] = '<a href="/course-category/video-courses/' . $course_cat->slug . '" title="' . esc_attr( $course_cat->name ) . '">' . $course_cat->name . '</a>';
					endif;
				else:
	...
	elseif ( is_archive() ) {
		...
		if ( is_tax() || is_category() || is_tag() ) {
			...
			if ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent ):
				...
				if(strpos($trail[1], 'title="VIDEO COURSES">VIDEO COURSES</a>') > 0):
					$trail[1] = '<a href="/library/" title="Library">Library</a>';
					$trail[2] = '<a href="/library/video-course-categories/" title="Video Course Categories">Video Course Categories</a>';
				endif;
			endif;
