...
class Client
{
    ...
    function getFile($path, $outStream, $rev = null)
    {
       ...
       function getPreview($path, $outStream, $rev = null)
    {
        Path::checkArgNonRoot("path", $path);
        Checker::argResource("outStream", $outStream);
        Checker::argStringNonEmptyOrNull("rev", $rev);
        ...
        $url = $this->buildUrlForGetOrPut(
            $this->contentHost,
            $this->appendFilePath("1/previews", $path),
            array("rev" => $rev));
    ...
    private function _uploadFileChunked($path, $writeMode, $inStream, $numBytes, $chunkSize)
    {
        ...
        while (!feof($inStream)) {
            ...
            while (true) {
                ...
                /* Update progress */
                $hash = $_REQUEST['hash'];
                $current = get_transient('outofthebox_upload_' . substr($hash, 0, 40));
                $percentage = ( round(($byteOffset / $numBytes) * 100) );
                $current['status'] = array('bytes_up_so_far' => $byteOffset, 'total_bytes_up_expected' => $numBytes, 'percentage' => $percentage, 'progress' => 'uploading');
                set_transient('outofthebox_upload_' . substr($hash, 0, 40), $current, HOUR_IN_SECONDS);
                /* End Update proccess */
                ...
                $serverByteOffset = $r;
                assert($serverByteOffset !== c);  // chunkedUploadContinue ensures this.
