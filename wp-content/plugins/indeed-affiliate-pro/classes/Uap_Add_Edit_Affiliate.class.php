...
if(!class_exists('Uap_Add_Edit_Affiliate')){
	class Uap_Add_Edit_Affiliate{
		...
		public function form(){
			...
			$i = 0;
			if ($this->register_template == 'uap-register-6'){
					...
					switch ($v['name']){
						...
						default:
							if ($this->is_public) {
								...
								$str .= '<div class="uap-form-line-register" id="' . $parent_id . '">';
				 ...
				 case 'uap-register-4':
				  ...
				  $add_class = '';
					if ($v['type'] == 'select' || $v['type'] == 'multi_select' || $v['type'] == 'file' || $v['type'] == 'upload_image' || $v['type'] == 'date'){
					 ...
					 $str .= '<div class="uap-form-line-register '.$add_class.'" id="' . $parent_id . '">';
					 if ($v['type'] == 'text' || $v['type'] == 'password'){
					 	if ($v['req']){
							 $placeholder .= '*';
						 }
		...
		private function set_mlm_parent(){
			 ...
			 * @return none
			 */
			if ($this->type=='create' && $this->is_public && get_option('uap_mlm_enable')){
				global $indeed_db;
