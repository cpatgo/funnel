...
if (!class_exists('Uap_Db')){
	class Uap_Db{
		...
		public function create_tables(){
			...
			/// VISITS
			$table_name = $wpdb->base_prefix . 'uap_visits';
			...
			/// BANNERS
			$table_name = $wpdb->base_prefix . 'uap_banners';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
							name VARCHAR(200),
			...
			/// NOTIFICATIONS
			$table_name = $wpdb->base_prefix . 'uap_notifications';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
							type VARCHAR(200),
			...
			/// RANKS
			$table_name = $wpdb->base_prefix . 'uap_ranks';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
							slug VARCHAR(200),
							...
							amount_value DECIMAL(8, 2) DEFAULT 0,
							bonus DECIMAL(8, 2) DEFAULT NULL,	
							sign_up_amount_value DECIMAL(8, 2) DEFAULT -1, 
							lifetime_amount_type VARCHAR(200) DEFAULT NULL,
							lifetime_amount_value DECIMAL(8, 2) DEFAULT -1,  
							reccuring_amount_type VARCHAR(200) DEFAULT NULL,
							reccuring_amount_value DECIMAL(8, 2) DEFAULT -1,	
			 ...
			 */
			$table_name = $wpdb->base_prefix . 'uap_referrals';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
			...
			/// OFFERS
			$table_name = $wpdb->base_prefix . 'uap_offers';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
			...
			$table_name = $wpdb->base_prefix . 'uap_offers_affiliates_reference';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
			...
			/// CAMPAIGNS
			$table_name = $wpdb->base_prefix . 'uap_campaigns';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
			 ...
			 */
			$table_name = $wpdb->base_prefix . 'uap_payments';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
			...
			/// MLM
			$table_name = $wpdb->base_prefix . 'uap_mlm_relations';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
			...
			$table_name = $wpdb->base_prefix . 'uap_affiliate_referral_users_relations';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
							affiliate_id int(11) NOT NULL,
			...
			/// ranks
			$table_name = $wpdb->base_prefix . 'uap_ranks_history';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
							affiliate_id int(11) NOT NULL,
			...
			$table_name = $wpdb->base_prefix . 'uap_landing_commissions';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
			...
			$table_name = $wpdb->base_prefix . 'uap_coupons_code_affiliates';
			if ($wpdb->get_var( "show tables like '$table_name'" ) != $table_name){
				$sql = "CREATE TABLE " . $table_name . " (
							id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
			...
			/// UAP_REPORTS
			$table_name = $wpdb->base_prefix . 'uap_reports';
			if ($wpdb->get_var("show tables like '$table_name'")!=$table_name){
				$sql = "CREATE TABLE $table_name (
							affiliate_id int(11) NOT NULL,
			...
			$table_name = $wpdb->base_prefix . 'uap_dashboard_notifications';
			if ($wpdb->get_var("show tables like '$table_name'")!=$table_name){
				$sql = "CREATE TABLE $table_name (
		...
		public function get_banners(){
			 ...
			 */
			global $wpdb;
			$table = $wpdb->base_prefix . "uap_banners";
		...
		public function save_banner($post_data=array()){
			...
			if (!empty($post_data)){
				...
				/// SAVE
				$wpdb->query("INSERT INTO $table  VALUES(NULL, '" . $post_data['name'] . "', '" . $post_data['description'] . "', '" . $post_data['url'] . "', '" . $post_data['image'] . "', '" . $post_data['status'] .  "', NULL);");
			}
		}
		...
		public function get_affiliate_id_by_username($user_name=''){
			...
			if ($user_name){
				global $wpdb;
				$q = $wpdb->prepare("SELECT uap.id FROM " . $wpdb->base_prefix . "uap_affiliates uap	  
		...
		public function get_wp_username_by_affiliate_id($affiliate_id=0){
			...
			if ($affiliate_id){
				...
				$data = $wpdb->get_row("SELECT u.user_login 
											FROM " . $wpdb->base_prefix . "uap_affiliates uap
											INNER JOIN " . $wpdb->base_prefix . "users u
											ON uap.uid=u.ID
		...
		public function save_affiliate($uid=0){
			...
			if ($uid){
				global $wpdb;
				$table = $wpdb->base_prefix . 'uap_affiliates';
				$q = $wpdb->prepare("SELECT * FROM $table WHERE uid=%d ;", $uid);
				$exists = $wpdb->get_row($q);
				if (!$exists){
					$q = $wpdb->prepare("INSERT INTO $table VALUES(NULL, %d, 0, NULL, 1);", $uid);
					$wpdb->query($q);
		...
		public function delete_affiliate_details($id=0){
			 ...
			 */
			 if ($id){
			 	global $wpdb;
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_affiliates WHERE id='" . $id . "';");
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_affiliate_referral_users_relations WHERE affiliate_id='$id' ");
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_campaigns WHERE affiliate_id='$id' ");
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_offers_affiliates_reference WHERE affiliate_id='$id' ");
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_payments WHERE affiliate_id='$id' ");
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_ranks_history WHERE affiliate_id='$id' ");
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_referrals WHERE affiliate_id='$id' ");
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_visits WHERE affiliate_id='$id' ");	
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_mlm_relations WHERE affiliate_id='$id' ");/// MLM child
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_mlm_relations WHERE parent_affiliate_id='$id' ");/// MLM parent
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_coupons_code_affiliates WHERE affiliate_id='$id' ");	
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_reports WHERE affiliate_id='$id';");						 
			 }
		}
		...
		public function affiliate_has_childrens($affiliate_id=0){
			 ...
			 */
			 if ($affiliate_id){
			 	 global $wpdb;
				 $table = $wpdb->base_prefix . 'uap_mlm_relations';
		...
		public function get_affiliates($limit=-1, $offset=-1, $count=FALSE, $order_by='', $order_type='', $where_conditions=array()){
			...
			$affiliates_table = $wpdb->base_prefix . 'uap_affiliates';
			$user_table = $wpdb->base_prefix . 'users';
			$users_meta_table = $wpdb->base_prefix . 'usermeta';
		
			if ($count){
				$q = "SELECT COUNT(distinct um.user_id) as c ";
			} else {
			...
			if (!empty($_GET['search_t'])){
				$search_term = $_GET['search_t'];
				$q .= " AND (
			...
			if ($count){
				...
				if (!empty($data)){
					foreach ($data as $obj){
						if ($obj->id){
							$user_data = get_userdata($obj->uid);
							$return_arr[$obj->id]['uid'] = $obj->uid;
							$return_arr[$obj->id]['username'] = $user_data->data->user_login;
							$first_name = get_user_meta($obj->uid, 'first_name', TRUE);
							$last_name = get_user_meta($obj->uid, 'last_name', TRUE);
							$return_arr[$obj->id]['name'] = $first_name . ' ' . $last_name;
							$return_arr[$obj->id]['email'] = $user_data->data->user_email;
							$return_arr[$obj->id]['start_data'] = $obj->start_data;
							$return_arr[$obj->id]['rank_label'] = '-';
							$return_arr[$obj->id]['rank_id'] = $this->get_affiliate_rank($obj->id);
							...
							$return_arr[$obj->id]['rank_color'] = (isset($rank_data['color'])) ? $rank_data['color'] : '';
							$user_object = new WP_User($obj->uid);
							$return_arr[$obj->id]['role'] = (isset($user_object->roles[0])) ? $user_object->roles[0] : '';
							$return_arr[$obj->id]['stats'] = $this->get_stats_for_payments($obj->id);
							$return_arr[$obj->id]['payment_settings'] = $this->get_affiliate_payment_type($obj->uid);
							$return_arr[$obj->id]['email_status'] = get_user_meta($obj->uid, 'uap_verification_status', TRUE); 
						}
					}
				}
			}	
		...
		public function get_affiliates_username_id_pair(){
			...
			$arr = array();
			global $wpdb;
			$table_affiliates = $wpdb->base_prefix . 'uap_affiliates';
		...
		public function delete_notification($id=0){
			...
			if (!empty($id)){
				global $wpdb;
				$table = $wpdb->base_prefix . "uap_notifications";
				$wpdb->query("DELETE FROM $table WHERE id='" . $id . "';");				
			}
		}
		...
		public function save_notification($post_data=array()){
			/*
			 * @param array ($_POST)
			 * @return none
			 */
			if (!empty($post_data)){
				...
				/// SAVE
				$wpdb->query("INSERT INTO $table  VALUES(NULL, '" . $post_data['type'] . "', '" . $post_data['rank_id'] . "', '" . $post_data['subject'] . "', '" . $post_data['message'] . "', '" . $post_data['status'] .  "');");
			}			
		}
		...
		public function register_get_custom_fields($only_public=FALSE, $exclude_fields=array()){
			...
			if ($data===FALSE){
				$defaults = array(
						array( 'display_admin'=>2, 'display_public_reg'=>2, 'display_public_ap'=>2, 'name'=>'user_login', 'label'=>'Username', 'type'=>'text', 'native_wp' => 1, 'req' => 1, 'sublabel' => '' ),
						...
						array( 'display_admin'=>1, 'display_public_reg'=>2, 'display_public_ap'=>1, 'name'=>'pass1', 'label'=>'Password', 'type'=>'password', 'native_wp' => 1, 'req' => 1, 'sublabel' => '' ),
						array( 'display_admin'=>1, 'display_public_reg'=>1, 'display_public_ap'=>1, 'name'=>'pass2', 'label'=>'Confirm Password', 'type'=>'password', 'native_wp' => 1, 'req' => 2, 'sublabel' => '' ),
		...
		public function return_settings_from_wp_option($type, $only_name=false, $return_default=false){
			...
			switch ($type){
				...
				break;
				case 'account_page':
					$arr = array(	'uap_ap_theme' => 'uap-ap-theme-2',
									...
									'uap_ap_welcome_msg' => '<span class="uap-user-page-mess-special">Hello</span> <span class="uap-user-page-name"> {last_name} {first_name}</span>,
															 <span class="uap-user-page-mess">you\'re logged as</span><span class="uap-user-page-mess-special"> {username}</span>
															 <span class="uap-user-page-mess">Your awesome e-mail address is : <strong>{user_email}</strong></span>
															 <span class="uap-user-page-mess">And your current rank is : <strong>{rank_name}</strong></span>',
									...
									'uap_tab_payments_settings_title' => __('Payments Settings', 'uap'),
									'uap_tab_payments_settings_menu_label' => __('Payments Settings', 'uap'),
				...
				case 'woo_account_page':
					$arr = array(
									...
									'uap_woo_account_page_non_affiliate_content' => '<p>' . __('How about become our affiliate?', 'uap') . '</p>[uap-user-become-affiliate]',
					);					
					break;
				case 'bp_account_page':
					$arr = array(
									...
									'uap_bp_account_page_non_affiliate_content' => '<p>' . __('How about become our affiliate?', 'uap') . '</p>[uap-user-become-affiliate]',
					);					
					break;
...
<div></div>
<div>Congratulations! You\'ve got a new Referral for your Account ({username}):</div>
<div></div>
...
<div></div>
<div></div>
<div>You can follow your Referrals int your Account Page:</div>
...
<div></div>
				...
				case 'periodically_reports':
					$arr = array(
									...
									'uap_periodically_reports_subject' => '{blogname} Periodically Reports for {username} Affiliate Account',
									'uap_periodically_reports_content' => '
<div>Hi {first_name} {last_name},</div>
<div></div>
<div>Here\'s a quick overview of your Affiliate Account from the last report:</div>
<div></div>
...
<div><strong>Refuse Referrals</strong> : {refuse_referrals}</div>									
		...
		public function get_ranks($only_active=FALSE){
			 ...
			 */
			global $wpdb;
			$table = $wpdb->base_prefix . 'uap_ranks';
			$q = "SELECT * FROM $table";
			if ($only_active){
				$q .= " WHERE status=1;";
			}
		...
		public function add_new_rank_to_history($uid=0, $prev_rid=0, $rid=0){
			...
			if ($uid && isset($rid)){
				global $wpdb;
				$table = $wpdb->base_prefix . 'uap_ranks_history';
				$q = $wpdb->prepare("INSERT INTO $table VALUES(null, %d, %d, %d, null);", $uid, $prev_rid, $rid);
				$wpdb->query($q);
			}
		}
		...
		public function get_last_rank_achievements($limit=50, $search='', $affiliate_id=0){
			...
			global $wpdb;
			$arr = array();
			$table_a = $wpdb->base_prefix . 'uap_ranks_history';
			$table_b = $wpdb->base_prefix . 'uap_affiliates';
		...
		public function delete_visits($delete_arr=array()){
			...
			if (!empty($delete_arr) && !empty($delete_arr)){
				global $wpdb;
				$db_name = $wpdb->base_prefix . "uap_visits";
		...
		public function track_the_visit($visitor_hash='', $referral_id=0, $affiliate_id=0, $url='', $ip='', $browser='', $device='', $campaign_name=''){
			 ...
			 */
			global $wpdb;
			$visit_table = $wpdb->base_prefix . "uap_visits";
			$campaign_table = $wpdb->base_prefix . 'uap_campaigns';
			...
			$q = $wpdb->prepare("INSERT INTO $visit_table 
									VALUES(null, %s, %d, %d, %s, %s, %s, %s, %s, null, null);", $visitor_hash, $referral_id, $affiliate_id, $campaign_name, $ip, $url, $browser, $device);
			$wpdb->query($q);
		...
		public function get_campaigns_reports_for_affiliate_id($id=0, $limit=-1, $offset=-1, $only_counts=FALSE, $order_by='', $order_type='', $where_conditions=array()){
			...
			if ($id){
				global $wpdb;
				$table = $wpdb->base_prefix . "uap_campaigns";
				if ($only_counts){
		...
		public function save_referral_from_admin($post_data=array()){
			 ...
			 */
			 if (!empty($post_data)){
			 	 global $wpdb;
				 $table = $wpdb->base_prefix . 'uap_referrals';
				 $table_b = $wpdb->base_prefix . 'uap_visits';
		...
		public function delete_referral($referral_id=0, $delete_child=TRUE, $delete_parent=TRUE){
			...
			if ($referral_id){
				...
				/// DELETE FROM VISITS
				$wpdb->query("DELETE FROM " . $wpdb->base_prefix . "uap_visits WHERE referral_id='$referral_id';");
			}			
		}
		...
		public function search_affiliate_id_for_current_user($referral_wp_id=0){
			...
			if ($referral_wp_id){
				global $wpdb;
				$table = $wpdb->base_prefix . 'uap_affiliate_referral_users_relations';
		...
		public function insert_affiliate_referral_user_new_relation($affiliate_id=0, $referral_wp_id=0){
			...
			if ($affiliate_id && $referral_wp_id){
				...
				$data = $wpdb->get_row($q);
				if (!$data){
					$q = $wpdb->prepare("INSERT INTO $table VALUES(NULL, %d, %d, NULL);", $affiliate_id, $referral_wp_id);
					$wpdb->query($q);
				}
			}
		}
		...
		public function get_offers(){
			...
			$return_data = array();
			global $wpdb;
			$table = $wpdb->base_prefix . "uap_offers";
			$table_b = $wpdb->base_prefix . "uap_offers_affiliates_reference";
			$data = $wpdb->get_results("SELECT * FROM $table;");
			if (!empty($data)){
				foreach ($data as $object){
		...
		public function delete_offers($ids){
			...
			if (!empty($ids)){
				global $wpdb;
				$table = $wpdb->base_prefix . "uap_offers";
				$table_offers_u_r = $wpdb->base_prefix . 'uap_offers_affiliates_reference';
				if (!is_array($ids)){
					$ids = array($ids);
				}
				foreach ($ids as $id){
		...
		public function get_offer_id_by_affiliate_id_and_source($affiliate_id=0, $source=''){
			...
			if ($affiliate_id){
				global $wpdb;
				$table = $wpdb->base_prefix . 'uap_offers_affiliates_reference';
				$table_offers = $wpdb->base_prefix . 'uap_offers';
		...
		public function search_affiliates_by_char($char=''){
			/*
			 * @param string
			 * @return array
			 */
			$return = array();
			global $wpdb;
			$data = $wpdb->get_results("SELECT u.user_login, uap.id 
											FROM ".$wpdb->base_prefix."users u
											INNER JOIN ".$wpdb->base_prefix."uap_affiliates uap
											ON u.ID=uap.uid
		...
		public function get_payments($limit=-1, $offset=-1, $count=FALSE, $order_by='', $order_type=''){
			...
			global $wpdb;
			$table = $wpdb->base_prefix . 'uap_referrals';
			$table_payments = $wpdb->base_prefix . 'uap_payments';
		...
		public function get_stats_for_payments($affiliate_id=0){
			...
			$temp_table = $wpdb->base_prefix . 'uap_visits';
		...
		public function set_mlm_relation_on_new_affiliate($affiliate_id=0){
			/*
			 * USE THIS WHEN AFFILIATE REGISTER FROM ANOTHER SYSTEM OR BECOME AFFILIATE BY SHORTCODE BUTTON
			 * @param int
			 * @return none
			 */
			 if (get_option('uap_mlm_enable') && $affiliate_id && isset($_COOKIE) && !empty($_COOKIE['uap_referral'])){
		...
		public function get_affiliate_user_relation($affiliate_username='', $user_username=''){
			...
			$table_a = $wpdb->base_prefix . 'uap_affiliates';
			$table_au = $wpdb->base_prefix . 'uap_affiliate_referral_users_relations';
		...
		public function get_stats_for_reports($time='', $affiliate_id=0){
			...
			if (!empty($start_time)){
				$q .= " AND update_date>'$start_time'";
			}
			if (!empty($end_time)){
				$q .= " AND update_date<'$end_time'";
			...
			if (!empty($start_time)){
				$q .= " AND a.start_data>'$start_time'";
			}
			if (!empty($end_time)){
				$q .= " AND a.start_data<'$end_time'";
			} else if (!empty($end_now)){
			...
			if (!empty($start_time)){
				$q .= " AND visit_date>'$start_time'";
			}
			if (!empty($end_time)){
				$q .= " AND visit_date<'$end_time'";
			} else if (!empty($end_now)){
		...
		public function payments_send_affiliate_notification_by_status($id=0, $status=0){
			 ...
			 */
			 global $wpdb;
			 $table_a = $wpdb->base_prefix . 'uap_payments';
			 $table_b = $wpdb->base_prefix . 'uap_affiliates';
		...
		public function get_landing_commissions(){
			 ...
			 */
			 global $wpdb;
			 $array = array();
			 $table = $wpdb->base_prefix . 'uap_landing_commissions';
			 $data = $wpdb->get_results("SELECT * FROM $table");
			 if ($data){
			 	foreach ($data as $object){
			 		$array[] = (array)$object;
		...
		public function save_landing_commission($post_data=array()){
			 ...
			 */
			 ...
			 if (empty($post_data['id'])){
				...
				$wpdb->query("INSERT INTO $table VALUES(null, '" . $post_data['slug'] . "', '$settings', NULL, '" . $post_data['status'] . "');");
				return 1;
			 } else {
			 	/// update
		...
		public function save_coupon_affiliate_pair($post_data=array()){
			 ...
			 */
			 global $wpdb;
			 $table = $wpdb->base_prefix . 'uap_coupons_code_affiliates';
			 $settings = array(
			 					'amount_type' => @$post_data['amount_type'],
								'amount_value' => @$post_data['amount_value'],			 									
			 );
			 ...
			 */
			 ...
			 switch ($type){
				...
				case 'edd':
					...
					break;
				case 'ump':
					$table = $wpdb->base_prefix . 'ihc_coupons';
			 ...
			 */
			 ...
			 if ($type_of_name){
				 ...
				 $q = "SELECT a.id as id, u.$type_of_name as name FROM 
				 			...
				 			WHERE 1=1
				 ";
				 if ($who!='' && strpos('-1', $who)===FALSE){ /// IF NOT ALL AFFILIATES
				 	$q .= " AND a.id IN($who)";
				 }
				 $data = $wpdb->get_results($q);
				 if ($data){
		...
		public function save_affiliate_report_settings($affiliate_id=0, $post_data=array()){
			 ...
			 */
			 if ($affiliate_id && $post_data){
				 global $wpdb;
				 $table = $wpdb->base_prefix . 'uap_reports';
		...
		public function increment_dashboard_notification($type=''){
			 ...
			 */
			global $wpdb;
			$table = $wpdb->base_prefix . 'uap_dashboard_notifications';
