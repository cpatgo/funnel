...
if (!class_exists('Affiliate_Account_Page')){ 
	class Affiliate_Account_Page{
		...
		public function __construct($uid, $affiliate_id){
			...
			$this->current_url = UAP_PROTOCOL . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
		...
		private function head(){
			...
			global $indeed_db;
			$data['labels'] = array(
									'overview' => $this->account_page_settings['uap_tab_overview_menu_label'],
									'edit_account' => $this->account_page_settings['uap_tab_edit_account_menu_label'],
									'change_pass' => $this->account_page_settings['uap_tab_change_pass_menu_label'],									
									'affiliate_link' => $this->account_page_settings['uap_tab_affiliate_link_menu_label'],
									'campaigns' => $this->account_page_settings['uap_tab_campaigns_menu_label'],
									'banners' => $this->account_page_settings['uap_tab_banners_menu_label'],
									'visits' => $this->account_page_settings['uap_tab_visits_menu_label'],
									'referrals' => $this->account_page_settings['uap_tab_referrals_menu_label'],
									'campaign_reports' => $this->account_page_settings['uap_tab_campaign_reports_menu_label'],
									'reports' => $this->account_page_settings['uap_tab_reports_menu_label'],
									'payments' => $this->account_page_settings['uap_tab_payments_menu_label'],
									'payment_settings' => $this->account_page_settings['uap_tab_payments_settings_menu_label'],
									'logout' => $this->account_page_settings['uap_tab_logout_menu_label'],
			);
			
			if ($indeed_db->is_magic_feat_enable('coupons')){
				$data['label']['coupons'] = $this->account_page_settings['uap_tab_coupons_menu_label'];
			}
			if ($indeed_db->is_magic_feat_enable('custom_affiliate_slug')){
				$data['label']['custom_affiliate_slug'] = $this->account_page_settings['uap_tab_custom_affiliate_slug_menu_label'];
			}			
			if ($indeed_db->is_magic_feat_enable('mlm')){
				$data['label']['mlm'] = $this->account_page_settings['uap_tab_mlm_menu_label'];
			}
			if ($indeed_db->is_magic_feat_enable('wallet')){
				$data['label']['wallet'] = $this->account_page_settings['uap_tab_wallet_menu_label'];
			}
			if ($indeed_db->is_magic_feat_enable('referral_notifications') || $indeed_db->is_magic_feat_enable('periodically_reports')){
				$data['label']['referral_notifications'] = $this->account_page_settings['uap_tab_referral_notifications_menu_label'];
			}
			...
			if ($data['listing_items'] && is_array($data['listing_items'])){
				...
				foreach ($data['listing_items'] as $k => $arr){
					if (isset($data['listing_items'][$k]->url)){						
						$data['listing_items'][$k]->url =  uap_create_affiliate_link($data['listing_items'][$k]->url, $param, $value);				
					}
				}
			}
