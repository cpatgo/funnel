<div style="width: 97%">
	...
	<div class="col-xs-3">
		<div class="uap-dashboard-top-box">
			...
			<div class="stats">
				<h4><?php echo round($data['stats']['unpaid_payments_value'], 2) . ' ' . $data['currency'];?></h4>
	...
	<div class="uap-box-content-dashboard uap-last-five" style="padding: 20px;">
		...
		<?php foreach ($data['last_referrals'] as $array):?>
			<div style="margin-bottom:10px;">
				...
				<span style="display: inline-block; vertical-align: text-top;"><?php echo '  '.$array['amount'] . ' ' . $array['currency'] . __(' for ', 'uap') .  '<strong>'.$array['affiliate_username'] .'</strong><br/>'. __(' on ', 'uap') . uap_convert_date_to_us_format($array['date']); ?></span>
			</div> 
		<?php endforeach;?>
	</div>
	<?php endif;?>
			...
			<div class="uap-dashboard-top-affiliate">
				...
				<?php foreach ($data['top_affiliates'] as $key=>$value): ?>
					<div class="uap-dashboard-top-affiliate-single">
					 ...
					 <div class="uap-top-count"><?php _e('Refferals', 'uap');?> <?php echo $value['referrals'];?> | <?php _e('Total Amount', 'uap');?> <?php echo $value['sum'] . ' ' . $data['currency']; ?> </div>
					</div>
					<?php $i++;?>
				<?php endforeach;?>	
			</div>
