...
$query = mysqli_query($GLOBALS["___mysqli_ston"], "select * from e_voucher where user_id = '$user_id' and mode = 0 ");
...
<table class="table table-bordered">
	...
	<tr>
		...
		<th class="text-center"><?=$Used_Id;?></th>
		<th class="text-center"><?=$Used_Date;?></th> 
	</tr>
	</thead>
	<tbody>
<?php	  
	...
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "select * from e_voucher where user_id = '$user_id' and mode = 0 LIMIT $start,$plimit ");  
...
<?php	
	...
	{ ?> 
		<li id="DataTables_Table_0_previous" class="paginate_button previous" aria-controls="DataTables_Table_0" tabindex="0">
			<a href="<?= "index.php?page=used_pin&p=".($newp-1);?>">Previous</a>
		</li>
	<?php 
	}
	for ($i=1; $i<=$pnums; $i++) 
	{ 
		...
		{ ?>
			<li class="paginate_button " aria-controls="DataTables_Table_0" tabindex="0">
				<a href="<?= "index.php?page=used_pin&p=$i";?>"><?php print_r("$i");?></a>
			</li>
			<?php 
		}
		else
	...
	{ ?>
		<li id="DataTables_Table_0_next" class="paginate_button next" aria-controls="DataTables_Table_0" tabindex="0">
	   		<a href="<?= "index.php?page=used_pin&p=".($newp+1);?>">Next</a>
	   </li>
	<?php 
	} 
	?>
