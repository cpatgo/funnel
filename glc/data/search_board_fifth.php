...
{ 
	...
	if($num > 0)
	{
		...
		{
			$user_id = $row['id_user'];
			$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM board_fifth WHERE pos1 = '$user_id' ");
			while($row = mysqli_fetch_array($query))
			{
				$board_id = $row['board_id'];
			}	 
			echo "<script type=\"text/javascript\">";
			echo "window.location = \"index.php?page=my_board_fifth&bbid=$board_id\"";
			echo "</script>";
		}		
	}
	else
	...
	if($board_id != '')
	{
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM board_fifth WHERE board_id = '$board_id' ");
...
<form name="myform" action="index.php?page=search_board_fifth" method="post">
...
{
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM board_break_fifth WHERE user_id = '$id' order by id desc limit 1 ");
	while($row = mysqli_fetch_array($query))
	{
		$level = $row['level'];
	}
