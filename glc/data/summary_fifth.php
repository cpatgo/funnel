...
if(isset($u_bbid) && !empty($u_bbid)):
	...
	if(!empty($board_break_id)):
		...
		$page = "index.php?val=my_board_first";
		$brdimg = 'board';	
	...
	for($g = 0; $g < $c; $g++)
	{
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM board_fifth WHERE board_id = '$b_b_id[$g]' AND mode = 0 ");
			while($row = mysqli_fetch_array($query))
			{
				...
				$bord_name_h_5 = my_bords_name($bd_name);
				$no_b = $g+1;

				print "
					<tr align=center>
						<td>$no_b</td>
						<td><a style=\"color:#0066BF;  text-decoration:underline;\" href=\"index.php?page=summary_fifth&bbid=$board_id\" >ID# $board_id</a>
						</td>
						<td>$date</td>
					</tr>";
			}
...
{
	$q = mysqli_query($GLOBALS["___mysqli_ston"], sprintf("select * from board_fifth where pos1 = %d and mode = 0", $id));
...
function step1($board_id)
{
	...
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_break_fifth where user_id = '$user_id' and id < (select id from board_break_fifth where board_b_id = '$board_id' and user_id = '$user_id') order by id desc limit 1");
	while($r = mysqli_fetch_array($q))
	{
		return $r['board_b_id'];
	}
...
{
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_fifth where board_id = '$board_id'");
	while($r = mysqli_fetch_array($q))
	{
		return $r['pos1'];	
	}
...
{
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_break_fifth where user_id = '$id' order by id desc limit 1 ");
	while($r = mysqli_fetch_array($q))
	{
...
function get_next_inserting_board($user_id)
{
	...
	do
	{
		...
		$qur = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_break_fifth where user_id = '$board_break_id' ");
...
function get_qualification_summary($id,$level,$u_bbb_id)
{
	...
	if($id != 0)
	{
		$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_fifth where board_id = '$u_bbb_id' ");
		...
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM board_break_fifth WHERE qualified_id = '$id' and level = '$level' $time_qur group by user_id  order by time ");
		while($row = mysqli_fetch_array($query))
		{
...
{
	$ki = $k = 1;
	$qur = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_fifth where board_id = '$u_bbid' ");
	...
	for($i = 1; $i < count($parent_arr); $i++)
	{
		...
		while($rrrrrr = mysqli_fetch_array($qu))
		{
			...
			$qur = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_fifth where board_id = '$brd_id[0]' ");
			while($row = mysqli_fetch_array($qur))
			{
				for($i = 1; $i < 16; $i++)
				{
