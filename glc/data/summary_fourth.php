...
if(isset($u_bbid) && !empty($u_bbid)):
	...
	if(!empty($board_break_id)):
		...
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM board_fourth WHERE board_id = '$board_break_id' ");
		...
		display($poss,$page,$img,$user_name,$parent_u_name,$name,$position,$date,$gender,$real_parent_u,$real_child,$bb_level,$real_info_time,$comming_board_username,$inserting_board,$brdimg);
		?>	
		 <div >
			<div id="popup">	
			</div>
	...
	for($g = 0; $g < $c; $g++)
	{
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM board_fourth WHERE board_id = '$b_b_id[$g]' AND mode = 0 ");
			while($row = mysqli_fetch_array($query))
			{
				...
				$bord_name_h_4 = my_bords_name($bd_name);
				$no_b = $g+1;

				print "
					<tr align=center>
						<td>$no_b</td>
						<td><a style=\"color:#0066BF;  text-decoration:underline;\" href=\"index.php?page=summary_fourth&bbid=$board_id\" >ID# $board_id</a> 
						</td>
						<td>$date</td>
					</tr>";
			}
...
{
	$q = mysqli_query($GLOBALS["___mysqli_ston"], sprintf("select * from board_fourth where pos1 = %d and mode = 0", $id));
...
function step1($board_id)
{
	...
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_break_fourth where user_id = '$user_id' and id < (select id from board_break_fourth where board_b_id = '$board_id' and user_id = '$user_id') order by id desc limit 1");
	while($r = mysqli_fetch_array($q))
	{
		return $r['board_b_id'];
	}
...
{
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_fourth where board_id = '$board_id'");
	while($r = mysqli_fetch_array($q))
	{
		return $r['pos1'];	
	}
...
{
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_break_fourth where user_id = '$id' order by id desc limit 1 ");
	while($r = mysqli_fetch_array($q))
	{
...
function get_next_inserting_board($user_id,$level)
{
	...
	do
	{
		...
		$qur = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_break_fourth where user_id = '$board_break_id' and level = '$level' ");
...
function get_qualification_summary($id,$level,$u_bbb_id)
{
	...
	if($id != 0)
	{
		$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_fourth where board_id = '$u_bbb_id' ");
		...
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM board_break_fourth WHERE qualified_id = '$id' and level = '$level' $time_qur group by user_id  order by time ");
		while($row = mysqli_fetch_array($query))
		{
...
{
	$ki = $k = 1;
	$qur = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_fourth where board_id = '$u_bbid' ");
	...
	for($i = 1; $i < count($parent_arr); $i++)
	{
		...
		while($rrrrrr = mysqli_fetch_array($qu))
		{
			...
			$qur = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_fourth where board_id = '$brd_id[0]' ");
			while($row = mysqli_fetch_array($qur))
			{
				for($i = 1; $i < 16; $i++)
				{
