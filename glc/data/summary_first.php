...
//DISPLAY BOARD HISTORY
	...
	<?php
		...
		for($g = 0; $g < $c; $g++)
		{
			$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM board WHERE board_id = '$b_b_id[$g]' AND mode = 0 ");
			while($row = mysqli_fetch_array($query))
			{
				...
				$bord_name_h_1 = my_bords_name($bd_name);
				$no_b = $g+1;
				...
				print "
					<tr align=center>
						<td>$no_b</th>
						<td>
							<a style=\"color:#0066BF; text-decoration:underline;\" href=\"index.php?page=summary_first&bbid=$board_id\" >ID# $board_id </a>
						</td>
						<td>$time</th>
					</tr>";
			}
...
{
	$q = mysqli_query($GLOBALS["___mysqli_ston"], sprintf("select * from board where pos1 = %d and mode = 0", $id));
...
function step1($board_id)
{
	...
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_break where user_id = '$user_id' and id < (select id from board_break where board_b_id = '$board_id' and user_id = '$user_id') order by id desc limit 1");
	while($r = mysqli_fetch_array($q))
	{
		return $r['board_b_id'];
	}
...
{
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board where board_id = '$board_id'");
	while($r = mysqli_fetch_array($q))
	{
		return $r['pos1'];	
	}
...
{ 
	if($type == 'B') { $imges = "b"; } // active
	if($type == 'C') { $imges = "d"; }  // block
	return $imges;
}
...
function get_next_inserting_board($user_id,$level)
{
	...
	do
	{
		...
		$qur = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_break where user_id = '$board_break_id' and level = '$level' ");
...
{
	$ki = $k = 1;
	$qur = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board where board_id = '$u_bbid' ");
	...
	for($i = 1; $i < count($parent_arr); $i++)
	{
		...
		while($rrrrrr = mysqli_fetch_array($qu))
		{
			...
			$qur = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board where board_id = '$brd_id[0]' ");
			while($row = mysqli_fetch_array($qur))
			{
				for($i = 1; $i < 16; $i++)
				{
