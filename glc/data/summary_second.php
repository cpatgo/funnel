...
//DISPLAY BOARD HISTORY
	...
	for($g = 0; $g < $c; $g++)
	{
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM board_second WHERE board_id = '$b_b_id[$g]' AND mode = 0 ");
			while($row = mysqli_fetch_array($query))
			{
				...
				$bord_name_h_2 = my_bords_name($bd_name);
				$no_b = $g+1;

				print "
					<tr align=center>
						<td>$no_b</td>
						<td><a style=\"color:#0066BF;  text-decoration:underline;\" href=\"index.php?page=summary_second&bbid=$board_id\" >ID# $board_id </a>
						</td>
						<td>$date</td>
					</tr>";
			}
...
{
	$q = mysqli_query($GLOBALS["___mysqli_ston"], sprintf("select * from board_second where pos1 = %d and mode = 0", $id));
...
function step1($board_id)
{
	...
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_break_second where user_id = '$user_id' and id < (select id from board_break_second where board_b_id = '$board_id' and user_id = '$user_id') order by id desc limit 1");
	while($r = mysqli_fetch_array($q))
	{
		return $r['board_b_id'];
	}
...
{
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select * from board_second where board_id = '$board_id'");
	while($r = mysqli_fetch_array($q))
	{
		return $r['pos1'];	
	}
