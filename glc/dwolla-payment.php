...
if($postback == 'success' && $status == 'Completed') 
{
	$query 	= mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM temp_users tu INNER JOIN memberships m ON m.membership = tu.membership WHERE id_user = $orderId ");
	...
	if($num == 1 ) {
		...
		mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO user_membership (user_id,payment_type,number,initial,current) VALUES ('".$user_id."' , '".$row["reg_way"]."', '', '".$row["id"]."' , '".$row["id"]."') ");
		
		$date = date('Y-m-d');
		mysqli_query($GLOBALS["___mysqli_ston"], "insert into wallet (id , date) values ('$user_id' , '$date') ");
		
		$dwolla_sql = "INSERT INTO dwolla 
		(user_id,signature,amount,checkoutId,status,clearingDate,transaction,destinationTransaction,postback) 
		VALUES 
		('".$user_id."' , '".$signature."' , '".$amount."', '".$checkoutId."' , '".$status."' , '".$clearingDate."', '".$transaction."' , 
		'".$destinationTransaction."', '".$postback."') ";
		mysqli_query($GLOBALS["___mysqli_ston"], $dwolla_sql);
		...
		$msg = "msg=User Registration Successfully Completed!";
	} else {
		$msg = "err=Error. Unsuccessful registration, there was un error with your membership!";
	}
} else {
	$msg = "err=Error. Unsuccessful payment, contact support!";
}
	
header( "Location: http://".$_SERVER['SERVER_NAME']."/glc/login.php?".$msg );
?>