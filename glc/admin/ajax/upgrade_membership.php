...
elseif($payment_method === 'echeck'):
    ...
    //Set order details
    $echeck_class->setOrder($user_id, $membership_details['membership'], $amount);
    //Process order
...
if((int)$payment_error !== 1):
    ...
    if($fields['upgrade_membership'] < 5):
        //Automatically upgrade user
        $upgraded_date = date('Y-m-d H:i'); 
        $status = 1;
        $membership_class->upgrade_membership($user_id, $fields['upgrade_membership']);
        $email_class->upgrade_membership($user, $user_membership['membership'], $membership_details['membership']);
    endif;

	$data = array(
		'user_id' => $user_id,
		'current_membership' 	=> $user_membership['initial'],
