...
print "<table width=600 hspace = 0 cellspacing=0 cellpadding=0 border=0>
	...
	print "<tr><td colspan=5>&nbsp;</td></tr><td colspan=5 height=30px width=400 class=\"message tip\"><strong>";
		if ($newp>1)
	{ ?>
		<a href="<?php echo "index.php?page=standard_board&p=".($newp-1);?>">&laquo;</a>
	<?php 
	}
	for ($i=1; $i<=$pnums; $i++) 
	{ 
		if ($i!=$newp)
		{ ?>
			<a href="<?php echo "index.php?page=standard_board&p=$i";?>"><?php print_r("$i");?></a>
			<?php 
		}
		else
		{
	...
	if ($newp<$pnums) 
	{ ?>
	   <a href="<?php echo "index.php?page=standard_board&p=".($newp+1);?>">&raquo;</a>
	<?php 
	} 	
	print "</table>";
}	
