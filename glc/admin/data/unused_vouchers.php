...
print "<table width=\"100%\" border=\"0\">";
	...
	if($totalrows != 0)
	{
		...
		if ($newp>1)
		{ ?>
			<a href="<?php echo "index.php?page=unused_vouchers&p=".($newp-1);?>">&laquo;</a>
		<?php 
		}
		for ($i=1; $i<=$pnums; $i++) 
		{ 
			if ($i!=$newp)
			{ ?>
				<a href="<?php echo "index.php?page=unused_vouchers&p=$i";?>"><?php print_r("$i");?></a>
				<?php 
			}
			else
			{
		...
		if ($newp<$pnums) 
		{ ?>
		   <a href="<?php echo "index.php?page=unused_vouchers&p=".($newp+1);?>">&raquo;</a>
		<?php 
		} 
	}
	else 
