...
{	
	...
	if($chk >0)
	{
		$error_username = "<font color=\"#FF0000\" size=\"2\"><b><center>User Id '$username' is already exist!</b></font>";
	} 
	else
	{
		...
		if($children < 2)
		{
			...
			$regtra = "<font color=\"#00274F\" size=\"3\"><b><center>User Registration Successfully Completed !</center></b></font>";
				
		}
		else 
		{ 
			$regtra = "<font color=\"#FF0000\" size=\"3\"><center>Selected virtual parent already have two child !</center></font>"; 
		}
	}
}
...
?>
<span align="center" style="color:#07537E; font-size:18px;"><strong><?php print $regtra.$error_username; ?></strong></span>
<?php
$sql = "select * from temp_users where type = 'A'";
...
<table class="table table-bordered">
	...
	$sql = "select * from temp_users where type = 'A' LIMIT $start,$plimit";
	$query = mysqli_query($GLOBALS["___mysqli_ston"], $sql);
	while($row = mysqli_fetch_array($query))
	{
		$srno++;	
		...
		echo "
			<tr>
				...
				<td>$username</td>
				<td>$name</td>
				<td class=\"input-medium\" style=\"padding-left:5px; width:120px;\">$date <br> $time</td>
				<td>$email</small></td>
				<td>$phone_no</small></td>"; ?>
				<td>
					<form action="index.php?page=cash_member" method="post">
	...
	?>
		...
		{ ?>
			<li id="DataTables_Table_0_previous" class="paginate_button previous">
				<a href="<?="index.php?page=cash_member&p=".($newp-1);?>">Previous</a>
			</li>
		<?php 
		}
		for ($i=1; $i<=$pnums; $i++) 
		{ 
			...
			{ ?>
				<li class="paginate_button ">
					<a href="<?="index.php?page=cash_member&p=$i";?>"><?php print_r("$i");?></a>
				</li>
				<?php 
			}
			else
		...
		{ ?>
		   <li id="DataTables_Table_0_next" class="paginate_button next">
				<a href="<?="index.php?page=cash_member&p=".($newp+1);?>">Next</a>
		   </li>
		<?php 
		} 
		?>
