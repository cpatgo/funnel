...
/*$to = "neeraj.alienweb.gmail.com";
generate_epin(1,5,$to,3,1);
$temp = 0;*/
function generate_epin($id,$new_user,$product_id,$to,$num,$data_log,$log_type,$from,$SmtpServer,$SmtpPort,$SmtpUser,$SmtpPass,$epin_generate_message,$fees) //$to = email id , num = no of unique id
{
	...
	for($i = 0; $i < $c; $i++)
	{
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "select * from e_pin where epin = '$unique_epin[$i]' ");
		...
		if($num == 0)
		{
			...
			$date = date('Y-m-d');
			$t = time();
			mysqli_query($GLOBALS["___mysqli_ston"], "insert into e_pin (epin, user_id , product_id , mode , time , date , used_id) values ('$unique_epin[$i]' , '$id' ,'$product_id' , '$mode' , '$t' , '$date' , '$new_user')");
			
			data_logs($id,$data_log[8][0],$data_log[8][1],$log_type[7]);
