...
tinymce.PluginManager.add('fontawesome', function (editor, url) {
    ...
    function showDialog() {
        ...
        function groupHtml(iconGroup, iconTitle) {
            ...
            var width = 12; // hwo many items per row
                ...
                for (var x = 0; x < width; x++) {
                    if (iconGroup[y * width + x]) {
                        if (iconTitle === 'Spinner') {
                            gridHtml += '<td><span class="fa fa-spin">' + iconGroup[y * width + x] + '</span></td>';//mceNonEditable
                        }
                        else {
                            gridHtml += '<td><span class="fa">' + iconGroup[y * width + x] + '</span></td>';//mceNonEditable
                        }
                    }
                }
