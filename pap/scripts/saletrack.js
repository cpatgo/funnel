...
var PostAffAction = function(actionCode) {
	...
	this._customCommissionNextTiersFromCampaign = '';
	this._status = '';
	this._currency = '';
	this._couponCode = '';
}
...
PostAffAction.prototype.setBannerID = function(value) {
	try {
		if (typeof (value) != 'undefined') {
			this._bannerID = this.correctTextChars(value);
		}
	} catch (err) {
	}
}
	...
	try {
		if (typeof (value) != 'undefined') {
			...
			for (i = 0; i < valueArray.length; i++) {
				this._customCommission += this.correctCustomParameterChars(valueArray[i]);
...
PostAffAction.prototype.setCustomCommissionNextTiersFromCampaign = function(value) {
	try {
		if (typeof (value) != 'undefined') {
			this._customCommissionNextTiersFromCampaign = value;
		}
	} catch (err) {
	}
}
...
PostAffTrackerObject.prototype._getParameters = function() {
	...
	if (length > 0) {
		for (i = 0; i < length; i++) {
			...
			if (obj._bannerId != null) {
				saleParams += 'bannerId='+obj._bannerId+';';
			}
			...
			if (obj._loadNextTiersFromCampaign != null) {
				saleParams += 'customCommmissionNextTiersFromCampaign='+obj._customCommissionNextTiersFromCampaign+';';
			}
			if (obj._status != null) {
			...
			escape(
					...
					'bannerId='+obj._bannerID + ';'+
					'campaignId='+obj._campaignID + ';'+
					'channelId='+obj._channelID + ';'+
					'customCommmission='+obj._customCommission + ';'+
					'customCommissionNextTiersFromCampaign='+obj._customCommissionNextTiersFromCampaign + ';'+
					'status='+obj._status + ';' +
