	 ...
	 /**
		...
		public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0) {
			...
			if (!in_array($file, $this->imagekeys)) {
				...
				if ($type == '') {
					...
					if (isset($fileinfo['mime']) AND (!empty($fileinfo['mime']))) {
						$type = substr($fileinfo['mime'], strpos($fileinfo['mime'], '/')+1);
					} else {
		...
		*/
		protected function _toJPEG($image) {
		    if (!$image) {
		        $this->Error('Error createing jpeg image: '.$file);
		        return false;
		    }
...
// bug, good url is replaced with bad path						
