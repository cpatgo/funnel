...
class Net_SMTP
{
    ...
    function _send($data)
    {
        ...
        if (PEAR::isError($error = $this->_socket->write($data))) {
            return PEAR::getInstance()->raiseError('Failed to write to socket: ' .
                                    $error->getMessage());
        }
    ...
    function _put($command, $args = '')
    {
        ...
        if (strcspn($command, "\r\n") !== strlen($command)) {
            return PEAR::getInstance()->raiseError('Commands cannot contain newlines');
        }
    ...
    function _parseResponse($valid)
    {
        ...
        while ($line = $this->_socket->readLine()) {
            ...
            if (empty($line)) {
                $this->disconnect();
                return PEAR::getInstance()->raiseError('Connection was unexpectedly closed');
            }
        ...
        return PEAR::getInstance()->raiseError('Invalid response code received from server',
                                $this->_code);
    }
    ...
    function connect($timeout = null, $persistent = false)
    {
        ...
        if (PEAR::isError($result)) {
            return PEAR::getInstance()->raiseError('Failed to connect socket: ' .
                                    $result->getMessage());
        }
    ...
    function disconnect()
    {
        ...
        if (PEAR::isError($error = $this->_socket->disconnect())) {
            return PEAR::getInstance()->raiseError('Failed to disconnect socket: ' .
                                    $error->getMessage());
        }
    ...
    function _negotiate()
    {
        ...
        if (PEAR::isError($this->_parseResponse(250))) {
            ...
            if (PEAR::isError($this->_parseResponse(250))) {
                return PEAR::getInstance()->raiseError('HELO was not accepted: ', $this->_code);
            }
    ...
    function _getBestAuthMethod()
    {
        ...
        return PEAR::getInstance()->raiseError('No supported authentication methods');
    }
    ...
    function auth($uid, $pwd , $method = '')
    {
        if (empty($this->_esmtp['AUTH'])) {
            if (version_compare(PHP_VERSION, '5.1.0', '>=')) {
                if (!isset($this->_esmtp['STARTTLS'])) {
                    return PEAR::getInstance()->raiseError('SMTP server does not support authentication');
                }
                ...
                if (PEAR::isError($result = $this->_socket->enableCrypto(true, STREAM_CRYPTO_METHOD_TLS_CLIENT))) {
                    ...
                    return PEAR::getInstance()->raiseError('STARTTLS failed');
                }
        ...
        if (empty($method)) {
            ...
            if (!in_array($method, $this->auth_methods)) {
                return PEAR::getInstance()->raiseError("$method is not a supported authentication method");
            }
        }
        ...
        default:
            $result = PEAR::getInstance()->raiseError("$method is not a supported authentication method");
            break;
        }
    ...
    function data($data)
    {
        ...
        if (isset($this->_esmtp['SIZE']) && ($this->_esmtp['SIZE'] > 0)) {
            if (strlen($data) >= $this->_esmtp['SIZE']) {
                $this->disconnect();
                return PEAR::getInstance()->raiseError('Message size excedes the server limit');
            }
        }
