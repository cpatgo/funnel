...
class Mail_smtp extends Mail {
    ...
    function send($recipients, $headers, $body)
    {
        ...
        if (is_object($this->_smtp) === false) {
            ...
            if (is_object($this->_smtp) === false) {
                return PEAR::getInstance()->raiseError('Failed to create a Net_SMTP object',
            ...
            if (PEAR::isError($res = $this->_smtp->connect($this->timeout))) {
                $error = $this->_error('Failed to connect to ' .
                                       $this->host . ':' . $this->port,
                                       $res);
                return PEAR::getInstance()->raiseError($error, PEAR_MAIL_SMTP_ERROR_CONNECT);
            }
            ...
            if ($this->auth) {
                ...
                if (PEAR::isError($res = $this->_smtp->auth($this->username,
                    ...
                    $error = $this->_error("$method authentication failure",
                                           $res);
                    $this->_smtp->rset();
                    return PEAR::getInstance()->raiseError($error, PEAR_MAIL_SMTP_ERROR_AUTH);
                }
            }
        }
		...
		if (isset($headers['Cc'])  && !empty($headers['Cc'])) {
            $allRecipients = $allRecipients .','. $headers['Cc'];
        }
        if (isset($headers['Bcc'])  && !empty($headers['Bcc'])) {
            $allRecipients = $allRecipients .','. $headers['Bcc'];
            unset($headers['Bcc']);
        }
        ...
        if (!isset($from)) {
            $this->_smtp->rset();
            return PEAR::getInstance()->raiseError('No From: address has been provided',
        ...
        if (PEAR::isError($res = $this->_smtp->mailFrom($from, $args))) {
            ...
            $this->_smtp->rset();
            return PEAR::getInstance()->raiseError($error, PEAR_MAIL_SMTP_ERROR_SENDER);
        }

        $recipients = $this->parseRecipients($allRecipients);
        ...
        foreach ($recipients as $recipient) {
            if (PEAR::isError($res = $this->_smtp->rcptTo($recipient))) {
                $error = $this->_error("Failed to add recipient: $recipient",
                                       $res);
                $this->_smtp->rset();
                return PEAR::getInstance()->raiseError($error, PEAR_MAIL_SMTP_ERROR_RECIPIENT);
            }
        }
        ...
        if (PEAR::isError($res = $this->_smtp->data($textHeaders . "\r\n\r\n" . $body))) {
            ...
            $this->_smtp->rset();
            return PEAR::getInstance()->raiseError($error, PEAR_MAIL_SMTP_ERROR_DATA);
        }
