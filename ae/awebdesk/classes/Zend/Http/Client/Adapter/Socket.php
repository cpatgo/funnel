...
require_once(awebdesk_classes('Zend/Uri/Http.php'));
/**
 * @see Zend_Http_Client_Adapter_Interface
 */
//require_once 'Zend/Http/Client/Adapter/Interface.php';
require_once(awebdesk_classes('Zend/Http/Client/Adapter/Interface.php'));
/**
 * @see Zend_Http_Client_Adapter_Stream
 */
//require_once 'Zend/Http/Client/Adapter/Stream.php';
require_once(awebdesk_classes('Zend/Http/Client/Adapter/Stream.php'));
...
class Zend_Http_Client_Adapter_Socket implements Zend_Http_Client_Adapter_Interface, Zend_Http_Client_Adapter_Stream
{
	...
	/**
 	...
 	/**
     * Set the stream assets for the TCP connection to the server
     *
     * Can accept either a pre-existing stream assets resource, or an array
     * of stream options, similar to the options array passed to the
     * stream_context_create() PHP function. In such case a new stream assets
     * will be created using the passed options.
     *
     * @since  Zend Framework 1.9
     *
     * @param  mixed $assets Stream assets or array of assets options
     * @return Zend_Http_Client_Adapter_Socket
     */
    public function setStreamassets($assets)
    {
        if (is_resource($assets) && get_resource_type($assets) == 'stream-assets') {
            $this->_assets = $assets;

        } elseif (is_array($assets)) {
            $this->_assets = stream_context_create($assets);

        } else {
            // Invalid parameter
            require_once 'Zend/Http/Client/Adapter/Exception.php';
            throw new Zend_Http_Client_Adapter_Exception(
                "Expecting either a stream assets resource or array, got " . gettype($assets)
            );
        }
    ...
    /**
     * Get the stream assets for the TCP connection to the server.
     *
     * If no stream assets is set, will create a default one.
     *
     * @return resource
     */
    public function getStreamassets()
    {
        if (! $this->_assets) {
            $this->_assets = stream_context_create();
        }
    ...
    public function connect($host, $port = 80, $secure = false)
    {
        ...
        if (! is_resource($this->socket) || ! $this->config['keepalive']) {
            $assets = $this->getStreamassets();
            if ($secure || $this->config['ssluseassets']) {
                if ($this->config['sslcert'] !== null) {
                    if (! stream_assets_set_option($assets, 'ssl', 'local_cert',
                                                    $this->config['sslcert'])) {
                        require_once 'Zend/Http/Client/Adapter/Exception.php';
                ...
                if ($this->config['sslpassphrase'] !== null) {
                    if (! stream_assets_set_option($assets, 'ssl', 'passphrase',
            ...
            if (! $this->socket) {
                $this->close();
                //require_once 'Zend/Http/Client/Adapter/Exception.php';
                require_once(awebdesk_classes('Zend/Http/Client/Adapter/Exception.php'));
                throw new Zend_Http_Client_Adapter_Exception(
