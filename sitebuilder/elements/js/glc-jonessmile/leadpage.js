...
(function(window, query, App, undefined) {
  ...
  var mobile = (/(iPhone|iPod|BlackBerry|Android)/)
    ...
    with(this) {
      ...
      if (!window.gapi) {
        !(function() {
          ...
          po.async = true;
          po.src = 'https://apis.google.com/js/plusone.js?onload=onPlusOneLoadCallback';
  ...
  (function() {
    ...
    //this.leadboxRe = /[^/][0-9]{6,}/;
    ///get the leadbox id from a prequalified url
    this.getLeadBoxId = function(href) {
      href = href.split('?')[0];
      href = href.split('/');
    ...
    this.getPageMeta = function() {
      return $('meta[name="leadpages-meta-id"]')
        .attr('content') || '';
    };

    this.getPageForm = function() {
      return $('meta[name="leadpages-form-id"]')
        .attr('content') || '';
    };
    ...
    this.getServerLocation = function() {
      ...
      host = $('meta[name="leadpages-url"]')
        .attr('content');
      //host = host && protocol == 'https:' ? protocol + host.replace(/^[^\/]+/, '') : host;  // That makes MSIE9&10 CTA links throw 404s because $('<a href="//blah">').get(0).protocol is ":", which isn't a protocol!!! Simpler version:
      host = host && protocol + host.replace(/^[^\/]+/, '');
    ...
    this.onClickOptin = function(leadbox_id) {
      if (mobile) {
        var location = this.createLeadBoxUrl(leadbox_id);
        // Since we're sending users to a page instead of displaying the
        // LeadBox, we have to disable window.onbeforeunload for pages that are
        // set up to throw the alert box up
        window.onbeforeunload = null;
        window.open(location, '_self');
      } else {
        this.showLeadBox(leadbox_id);
      }
    };

    this.getLeadBoxWindow = function(leadbox_id) {
      var win = null;
      try {
        win = leadbox_id ? this.leadboxes[leadbox_id].get(0)
          .contentWindow : this.leadbox.get(0)
          .contentWindow;
      } catch (e) {}
      return win;
    };

    this.resizeLeadBox = function(leadbox_id) {
      var server = this.getServerLocation();
      var leadbox = leadbox_id ? this.leadboxes[leadbox_id] : this.leadbox;
      var leadboxWindow = this.getLeadBoxWindow(leadbox_id);
    ...
    this.showLeadBox = function(leadbox_id) {
      if (leadbox_id) {
        this.leadboxes[leadbox_id].attr('data-shown', 'true');
      } else {
        this.leadbox.attr('data-shown', 'true');
      }
      this.resizeLeadBox(leadbox_id);
    };
    ...
    this.hideLeadBox = function() {
      ...
      if (!this.isEmpty(this.leadboxes)) {
        for (key in this.leadboxes) {
          if (this.leadboxes[key].attr('data-shown')) {
            leadbox_id = key;
          }
          this.leadboxes[key].attr('data-shown', '')
            .removeAttr('data-shown');
        }
      } else {
        this.leadbox.attr('data-shown', '')
          .removeAttr('data-shown');
      }
      this.resizeLeadBox(leadbox_id);
    };

    this.disableExitPopup = function(e) {
      popup = false;
    };
    ...
    this.setupLeadBoxes = function() {
      ...
      var disableExitPopup = this.disableExitPopup.bind(this);
      var self = this;
      ...
      $('a')
        .each(function() {
          ...
          if (!href.match(/^https?:\/\//)) {
            var optin = href.match(/\/leadbox\/[\w]+:[\w]+\/([0-9]+)/);
            if (optin && optin.length > 1 && !!optin[1]) {
              $element.attr('data-optin', 'true');

              if (!self.leadboxes[optin[1]]) {
                location = self.createLeadBoxUrl(optin[1]);
                var lb = $('<iframe />')
                  .addClass('leadbox')
                  .attr('frameborder', 0)
                  .attr('src', location)
                  .attr('name', optin[1]); ///chrome bug fix
              } else {
                var lb = self.leadboxes[optin[1]];
              }
              self.leadboxes[optin[1]] = lb;
              if (!mobile) {
                setTimeout(function() { resizeLeadBox.apply(self, [optin[1]]) }, 1250);
                lb.appendTo('body');
                resizeLeadBox(optin[1]);
                winwin.resize(resizeLeadBox.apply(this, [optin[1]]));
              }
            } else {
              if (typeof window.top.App !== 'function') {
                if (href.indexOf('#') === 0) {
                  /// this is a hash link, so let's reset the href
      ...
      $('[data-linked]')
        ...
        .each(function() {
          var $element = $(this);
          var href = $(this)
            .attr('data-linked') || window.location.href;
          var frame = $(this)
            .attr('data-frame') || '_self';

          // explicitly setup the leadbox_id without breaking the method
          var leadbox_id = null;
          // Since leadboxRe is a very broad match it can inadvertently mistake external links for LeadBoxes.
          // We need to make another check to validate that this fake button actually is a LeadBox.
          if (href.match(/\/leadbox\/[\w]+:[\w]+\/([0-9]+)/)) {
            leadbox_id = self.getLeadBoxId(href);
            //if (self.leadboxRe.exec(href)) leadbox_id = self.leadboxRe.exec(href)[0];
          }
          ...
          $element.on('click', function(event) {
            ...
            return leadbox_id ? onClickOptin(leadbox_id) : window.open(href, frame);
          });

          if (leadbox_id) {
            $element.attr('data-optin', 'true');

            if (!self.leadboxes[leadbox_id]) {
              location = self.createLeadBoxUrl(leadbox_id);
              var lb = $('<iframe />')
                .addClass('leadbox')
                .attr('frameborder', 0)
                .attr('src', location);
            } else {
              var lb = self.leadboxes[leadbox_id];
            }
            self.leadboxes[leadbox_id] = lb;
            if (!mobile) {
              setTimeout(function() { resizeLeadBox.apply(self, [leadbox_id]) }, 250);
              lb.appendTo('body');
              resizeLeadBox(leadbox_id);
              winwin.resize(resizeLeadBox.apply(this, [leadbox_id]));
            }
          }
        });
      ...
      winwin.on('message', function(e) {
        switch (e.originalEvent.data) {
          case 'close':
            //var leadbox_id = self.leadboxRe.exec(e.target.location.hash);
            //leadbox_id = leadbox_id ? leadbox_id[0] : leadbox_id;
            hideLeadBox(); //leadbox_id);
            break;
    ...
    this.setupResponsives = function() {
      ...
      $(window)
        .on('resize', beResponsive);
      $(window)
        .on('load', beResponsive);
      $(window.document)
        .on('lp-refresh', beResponsive);
      if (query) query('html')
        .trigger('lpready');
    ...
    this.setupElementBehaviors = function() {
      ...
      $('[data-optin]')
        .filter(function() {
          var $element = $(this);
          $element.on('click', function(event) {
            var href = event.currentTarget.href || event.target.hash || event.target.href;
            href = href || (event.currentTarget.attributes['data-linked'] && event.currentTarget.attributes['data-linked'].value);
            href = href || this.href;
            if (href) {
              var leadbox_id = self.getLeadBoxId(href);//self.leadboxRe.exec(href);
              //leadbox_id = leadbox_id ? leadbox_id[0] : leadbox_id;
              event.stopPropagation();
              event.preventDefault();
              onClickOptin(leadbox_id);
              return false;
            }
            return true;
  ...
  $(function() {
    ...
    function confirmFont(haystack, needle) {
      var customCss = String(haystack || '')
        .toLowerCase();
      var fontName = String(needle || '')
        .toLowerCase();
...
.call(
  (window.LPFrame && window.LPFrame.context || window), (window.LPFrame && window.LPFrame.context || window), (window.LPFrame && window.LPFrame.$), (window.LPFrame && window.LPFrame.context ? window.App : null)
);
