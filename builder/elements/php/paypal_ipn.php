<?php
// Paypal IPN Listener for GLC Builder Paypal Notification
...
//Must end on first column
		...
		$subject = "GLC Builder Fraud Notification"; //insert here the subject of the email
	...
	$subject = "GLC Builder Paypal Notification"; //insert here the subject of the email
...
//Must end on first column
	...
	// PHP email error log
	if(!$buyer_mail) {
		error_log(date('[Y-m-d H:i e] '). "GLC Builder Paypal Notification to $payer_email failed". PHP_EOL, 3, LOG_FILE);
	} else {
		error_log(date('[Y-m-d H:i e] '). "GLC Builder Paypal Notification to $payer_email has been successfully sent". PHP_EOL, 3, LOG_FILE);
	}

	if(!$merchant_mail) {
		error_log(date('[Y-m-d H:i e] '). "GLC Builder Paypal Notification to $receiver_email for $payer_email purchase failed". PHP_EOL, 3, LOG_FILE);
	} else {
		error_log(date('[Y-m-d H:i e] '). "GLC Builder Paypal Notification to $receiver_email for $payer_email has been successfully sent". PHP_EOL, 3, LOG_FILE);
	}
