...
editableItemOptions['h6 : font-family'] = ['default', 'Lato', 'Helvetica', 'Arial', 'Times New Roman'];
...
$(function () {
    ...
    saveTemplateModalForm = function() {
    ...
    $("#xsExport").on('click', function (e) {
        var xsExpFile = new Object(),
                jsonData = [],
                i = 0;
        savePage(e, xsExpFile);
        for (ar in xsExpFile) {
            jsonData[i] = xsExpFile[ar];
            i++;
        }

        $("#xsExportData").val(JSON.stringify(jsonData));
...
function savePage(e, xsExpFile) {

    var xsExpFile = xsExpFile || null;
    ...
    $('#pageList > ul').each(function () {
        ...
        localStorage.removeItem("blocksFrame" + pageCounter);	//ar1
        ...
        if (xsExpFile != null) { //ar1
            xsExpFile['blocksElement' + pageCounter] = blocksElement;
            xsExpFile['blocksFrame' + pageCounter] = blocksFrame;
        }
        pageCounter++;
