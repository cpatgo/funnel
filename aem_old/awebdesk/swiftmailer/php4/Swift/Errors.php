...
class Swift_Errors
{
  ...
  function dumpError(&$e)
  {
    ...
    if ( isset($GLOBALS['_swift_campaignid']) and isset($GLOBALS['_swift_action']) ) {
      campaign_sender_log("[+] SwiftMailer encountered an error!");
      campaign_sender_log(strip_tags(str_replace("<br />", "\n", $output)));
      campaign_log_save($GLOBALS['_swift_campaignid'], $GLOBALS['_swift_action']);
    }
